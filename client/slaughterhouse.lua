...
local function SellWalkingAnimals()
    ...
    for _, animal in pairs(walkingAnimals) do
        ...
        if animal.adult == true then
            ...
            notify('Zvíře není dostatečně staré k prodeji.', 'error')
        end
    end
end
    ...
    while true do
        ...
        for v, npc in pairs(Config.SlaughterHouses) do
            ...
            if table.count(walkingAnimals) > 0 then
                -- print('Zvířata k prodeji: ' .. table.count(walkingAnimals))
                if dist < 3.0 then
                    pause = 0
                    if dist < 1.5 then
                        if Prompt == nil then
                        ...
                        if tempTool == Config.leashItem then
                            PromptSetActiveGroupThisFrame(promptGroup,
                                CreateVarString(10, 'LITERAL_STRING', "Prodat zvířata"))
