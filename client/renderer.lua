...
--         for _, railing in pairs(railings) do
--             local railingPos = railing.coords
--             local distance = #(playerPos - vector3(railingPos.x, railingPos.y, railingPos.z))

--             -- Inicializace stavu
--             if railing.active == nil then
--                 railing.active = false
--                 railing.lastSeen = 0
--                 railing.products = railing.products or {}
--             end

--             if distance <= Config.RenderDistance then
--                 railing.lastSeen = now

--                 if not railing.active then
--                     railing.active = true
--                 end
...
--                         if not DoesEntityExist(animal.obj) and animal.home == 1 then
--                             local anima = {}
--                             anima.model = (animal.gender == "female")
--                                 and Config.Animals[animal.breed].model
--                                 or Config.Animals[animal.breed].m_model
--                             anima.health = animal.health
--                             local pos = getRandomPosInRailing(railing) or railingPos

--                             TriggerEvent("aprts_ranch:Client:spawnAnimal", anima, pos, true, animal.id, railingPos)
--                             Citizen.Wait(200) -- malá prodleva mezi spawny
--                         elseif DoesEntityExist(animal.obj) then
--                             local health = GetEntityHealth(animal.obj)
--                             if health <= 0 and animal.health > 0 then
--                                 local cause = GetPedCauseOfDeath(animal.obj)
--                                 for key, val in pairs(Config.WeaponHash) do
--                                     if cause == val then
--                                         cause = key
--                                         break
--                                     end
--                                 end
--                                 local coords = GetEntityCoords(animal.obj)
--                                 TriggerServerEvent("aprts_clue:Server:addClue",
--                                     "Zvíře bylo zraněno zbraní: " .. cause,
--                                     coords, "p_cougarbloodpools01x", 5.0, 1)
--                                 TriggerServerEvent("aprts_ranch:Server:logKillAnimal", animal.id, cause, coords)
--                                 TriggerServerEvent("aprts_ranch:Server:killAnimal", animal.id)
--                             elseif animal.health == 0 then
--                                 SetEntityHealth(animal.obj, 0)
--                             end
...
--             else
--                 -- mimo vzdálenost, zkontroluj, jestli je čas despawnout
--                 if railing.active and (now - railing.lastSeen) >= (DESPAWN_DELAY * 1000) then
--                     railing.active = false
...
--                     -- 🧹 Despawn zvířat
--                     for _, animal in pairs(animals) do
--                         if animal.railing_id == railing.id and DoesEntityExist(animal.obj) then
--                             DeleteEntity(animal.obj)
--                         end
--                     end
--                 end
