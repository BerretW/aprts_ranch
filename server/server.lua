...
local function setSickAnimalsOnRailing(railing_id)
    for _, animal in pairs(animals) do
        if animal.railing_id == railing_id and animal.sick > 0 then
            if not animal.adult and animal.health > 0 then
                animal.sick = math.random(1, 50)
                TriggerClientEvent('aprts_ranch:Client:updateAnimal', -1, animal)
                MySQL:execute("UPDATE aprts_ranch_animals SET sick = ? WHERE id = ?", {animal.sick, animal.id})
                DiscordWeb("Ranch", "Mladé zvíře " .. animal.id .. " onemocnělo!", "Ranch")
                break
            end
        end
    end
        ...
        if product.gather == 3 and chance < product.chance then
            if product.gender == animal.gender or product.gender == nil then
                ...
                if railing.products[product.item] then
                    ...
                    railing.products[product.item].amount = math.min(railing.products[product.item].amount,
                        Config.MaxProduct * getCountAnimlsonRailing(railing))
    ...
    if animalConfig then
        ...
        if animal.age >= animalConfig.dieAge then
            ...
            if math.random() <= 0.2 then
                setSickAnimalsOnRailing(animal.railing_id)
            end
            return
        end
            ...
            for _, animalInRailing in pairs(animalsInsRailing) do
                if animalInRailing.sick == 0 then
                    ...
                    if math.random() < infectionChance then
                        ...
                        DiscordWeb("Ranch", "Zvíře " .. animalInRailing.id .. " se nakazilo od jiného zvířete!",
                            "Ranch")
...
function saveAnimalData(animal)
    ...
    pendingUpdates[animal.id] = {
        ...
        age = animal.age,
        sick = animal.sick,
        happynes = animal.happynes,
        energy = animal.energy,
        pregnant = animal.pregnant,
        count = animal.count,
        updated = animal.updated
    }
    -- print("Zvíře " .. animal.id .. " bylo uloženo do tabulky.")
end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(60000) -- každou minutu
        local ids = {}
        ...
        if #ids > 0 then
            ...
            local sql = [[
        UPDATE aprts_ranch_animals SET
          food    = ]] .. buildCase("food") .. [[,
          water   = ]] .. buildCase("water") .. [[,
          health  = ]] .. buildCase("health") .. [[,
          clean   = ]] .. buildCase("clean") .. [[,
          age     = ]] .. buildCase("age") .. [[,
          sick    = ]] .. buildCase("sick") .. [[,
          happynes= ]] .. buildCase("happynes") .. [[,
          energy  = ]] .. buildCase("energy") .. [[,
          pregnant= ]] .. buildCase("pregnant") .. [[,
          count   = ]] .. buildCase("count") .. [[,
        ...
        WHERE id IN (]] .. table.concat(ids, ",") .. [[);
            ...
            -- jeden db call pro všechno
            MySQL:execute(sql, {}, function(...)
            end)
            -- print("Aktualizace zvířat proběhla úspěšně. Počet aktualizovaných zvířat: " .. #ids)
            -- vyčisti buffer
            pendingUpdates = {}
        end
    end
