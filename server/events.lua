...
AddEventHandler("onResourceStart", function(resource)
    ...
    if resource == GetCurrentResourceName() then
        ...
        MySQL:execute(
            ...
            {}, function(result)
                for k, v in pairs(result) do
                    local animalConfig = Config.Animals[v.breed]
                    v.coords = json.decode(v.coords)
                    v.meta = json.decode(v.meta)
                    v.updated = v.updated
                    v.age = v.ageNow
                    v.adult = v.ageNow >= animalConfig.adultAge
                    v.injections = 0
                    -- print(v.adult, v.id, v.ageNow, animalConfig.adultAge)
    ...
    if animal then
        if railing then
            ...
            MySQL:execute(
                "UPDATE aprts_ranch_animals SET railing_id = ?, home = ?, coords = ?, health = ?, food = ?, water = ?, clean = ?, sick = ?, happynes = ?, energy = ?, age = ?, quality = ?, count = ?, born = FROM_UNIXTIME(?), pregnantStart =FROM_UNIXTIME(?), updated = FROM_UNIXTIME(?),xp= ?, meta = ? WHERE id = ?",
                {railingID, 1, json.encode(railing.coords), animal.health, animal.food, animal.water, animal.clean,
                 animal.sick, animal.happynes, animal.energy, animal.age, animal.quality, animal.count, animal.born, -- Should now be in seconds
...
AddEventHandler("aprts_ranch:Server:getData", function()
    ...
    TriggerClientEvent("aprts_ranch:Client:getData", _source, ranches, railings, animals, poops, Config.Animals,
        lostAnimals)
end)
...
AddEventHandler("aprts_ranch:Server:waterAnimal", function(animalID, waterAmount)
    ...
    local animal = animals[animalID]
    if not animal then
        notify(_source, "Zvíře nebylo nalezeno.")
        return
    end

    local hasItem = exports.vorp_inventory:getItemCount(_source, nil, Config.fullWaterItem)
    if hasItem < 1 then
        notify(_source, "Nemáš vodu, kterou bys zvíře napojil.")
        return
    end
    ...
    -- inicializace kapacity, pokud není definovaná
    if not meta.capacity then
        meta.capacity = Config.WateringCount
    end
    ...
    meta.description = "Zbývá " .. meta.capacity
        ...
        notify(_source, "Jsi vyprázdnil kýbl dosucha.")
    else
        exports.vorp_inventory:setItemMetadata(_source, item.id, meta)
        notify(_source, "Zbývá " .. meta.capacity .. " vody na zalévání v kýblu.")
    end
    ...
    if animal then
        ...
        for _, product in pairs(configAnimal.product) do
            if product.gather == 1 then
                ...
                print(animal.age, animal.adult, count)
                if animal.adult == false then
                    count = 1
                end
                count = math.floor(count)
...
AddEventHandler("aprts_ranch:Server:cureAnimal", function(animalID, cure)
    ...
    if animal.health <= 0 then
        notify(_source, "Zvíře je těžce raněné, musíš na to jinak!")
        return
    end
    if animal then
...
AddEventHandler("aprts_ranch:server:sellAnimal", function(animal)
    ...
    if animal then
        ...
        lib.logger(_source, 'RanchSellAnimal', "Hráč " .. playerName .. " prodal zvíře druhu " .. animal.breed ..
            "na krmítku: " .. animal.railing_id, "breed:" .. animal.breed, "railing_id:" .. animal.railing_id)
    else
        notify(_source, "Animal not found.")
    end
end)
